...
function authenticate_post()
	...
	redirect_to(redirect)
        ...
        --local digest = bcrypt.digest(post["password"], round_count)
        local digest = post["password"]
	-- Lua hashes strings, so these comparisons are time invariant.
	if password == nil or not bcrypt.verify(digest, password) then
		set_cookie("cgitauth", "")
	else
